...
class ObjectCounter1 implements Runnable {
...
class ObjectIncrement1 implements Runnable {
    ObjectCounter1 threadToCount;
    int objectFreq;
    int simulationSize;

    ObjectIncrement1 (ObjectCounter1 _threadToCount, int _objectFreq, int _simulationSize) {
...
class ObjectDecrement1 implements Runnable {
    ObjectCounter1 threadToDiscount;
    int objectFreq;
    int simulationSize;

    ObjectDecrement1(ObjectCounter1 _threadToCount, int _objectFreq, int _simulationSize) {
...
public class example2212 {
    public static void main(String[] args) throws InterruptedException {
        int simulationSize = 100;

        ObjectCounter1 _threadA = new ObjectCounter1();
        Thread threadA = new Thread(_threadA);
        threadA.setName("A");

        ObjectIncrement1 _threadB = new ObjectIncrement1(_threadA, 5, simulationSize);
        Thread threadB = new Thread(_threadB);
        threadB.setName("B");

        ObjectDecrement1 _threadC = new ObjectDecrement1(_threadA, 10, simulationSize);
